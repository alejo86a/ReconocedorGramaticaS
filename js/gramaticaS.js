 ...
 *repetidos.Es de gran importancia este vector, ya que nos determina el tamaño de rRecursivo y nos indica la this.posicion correcta 
...
function reconocimientoDecendente(){  //Crea el codigo recursivo del reconocimiento decendente
	...
	for (var j = 0; j < vectorNT.length; j++) {      //Recorro nT , variable J
		var txt="";
		txt="function metodo"+j+"(this.texto,this.pos)"+"{"+"\n"+"switch(this.texto[this.pos]){"+"\n"; // casos de simbolo
        	...
        	if (this.producciones[k].izq[1]===vectorNT[j]) {
               ...
               var bool=0;
                 for (var l = 1; l <prodAux.length ; l++) {                  //Se recorre la produccion derecha variable l
                 	...
                 	if(prodAux[l]!="<"){
                 		cont=cont+1;
 						txt=txt+"if(this.texto[this.pos]== " + prodAux[l] + " ) {"+"\n" + "this.pos=this.pos+1;"+"\n"; //lea(simbolo)
 						bool=1;
 						
                 	}
                 	if (prodAux[l]=="<") {
                 		txt=txt+"metodo"+this.posNTenVector(prodAux[l+1],vectorNT)+"(this.texto,this.pos);"+"\n";//nt+prodAux[l+1]()
                 		l=l+2;
                 	};
...
txtPrinci=txtPrinci+"\n"+"}"+"\n"+"function esHileraValida(this.texto){"+"\n"+
	"this.pos=0;"+"\n"+"metodo0(this.texto,this.pos);"+"\n"+"if(this.texto[this.pos]=="+"'¬'"+"){"+
	"return true;}else{return false;}"+"\n"+"}"+"\n"+"function setHilera(t){"+"\n"+
	"this.texto = t;"+"\n"+"}";
rRecursivo[vectorNT.length]=txtPrinci;
console.log(rRecursivo[vectorNT.length]);
